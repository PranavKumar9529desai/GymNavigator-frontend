...
export const MealSchema = z.object({
	...
	name: z
		...
		.min(
			5,
			'Meal name should be descriptive of the actual dish, not just "Breakfast" or "Lunch"',
		),
				...
				typedData.meals = typedData.meals.map((meal) => {
					if (meal?.name) {
						...
						if (
							genericNames.some(
								(generic) =>
									meal.name?.toLowerCase().includes(generic) &&
									meal.name?.trim().length < 15,
							)
						) {
							...
							if (meal.instructions && typeof meal.instructions === 'string') {
								...
								const foodItems = meal.instructions.match(
									/prepare|have|enjoy|make|cook|eat|with\s+([^.,:;]+)/i,
								);
								if (foodItems?.[0]) {
