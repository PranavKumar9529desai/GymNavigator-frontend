 ...
 */
export function buildGroceryListPrompt(dietPlan: DietPlan[], timeFrame: 'weekly' | 'monthly') {
  ...
  const multiplier = timeFrame === 'weekly' ? 1 : 4;
 ...
 */
export function validateGroceryList(data: unknown): { valid: boolean; data?: GroceryListResponse; errors?: unknown } {
  try {
    ...
    if (typeof data === 'string') {
      ...
      try {
        ...
        return { valid: false, errors: `Invalid JSON format after preprocessing: ${parseError.message}` };
      }
    }
    ...
    for (const category of typedData.categories) {
      if (!category.id || !category.name || !Array.isArray(category.items)) {
        return { valid: false, errors: `Invalid category: ${JSON.stringify(category)}` };
      }
      
      for (const item of category.items) {
        if (!item.name || typeof item.quantity !== 'number' || !item.unit) {
          return { valid: false, errors: `Invalid item in category ${category.name}: ${JSON.stringify(item)}` };
        }
      }
    }
