...
export function ErrorState({ errorMessage, gymName, gymId, hash, onRetry }: ErrorStateProps) {
  const router = useRouter();
  const [isRetrying, setIsRetrying] = useState(false);

  // Helper function to determine what kind of error we're dealing with
  const getErrorType = (): 'expired' | 'invalid' | 'connection' | 'permission' | 'unknown' => {
    ...
    if (msg.includes('expired') || msg.includes('timeout')) {
      ...
      return 'invalid';
    } else if (msg.includes('connection') || msg.includes('connect') || msg.includes('network')) {
      return 'connection';
    } else if (msg.includes('permission') || msg.includes('access') || msg.includes('not authorized')) {
      return 'permission';
    }
            ...
            <AccordionItem value="common-issues">
              ...
              <AccordionContent>
                <ul className="list-disc pl-5 space-y-2 text-sm">
                  ...
                  <li>Your gym administrator might need to add you to the system first</li>
            ...
            <AccordionItem value="contact-support">
              ...
              <AccordionContent>
                <p className="text-sm mb-2">
                  If you continue to experience issues, please contact your gym administrator 
                  or reach out to our support team.
                </p>
                <Button variant="outline" size="sm" className="w-full mt-2" onClick={() => window.location.href = '/help'}>
                  Go to Help Center
                </Button>
              </AccordionContent>
            </AccordionItem>
