...
export function PlanCard({ plan, isSelected, onSelect }: PlanCardProps) {
  ...
  const availabilityPercentage = plan.maxMembers && plan.currentMembers 
    ? Math.round(((plan.maxMembers - plan.currentMembers) / plan.maxMembers) * 100)
    : 100;
    ...
    <TooltipProvider>
      ...
      >
        ...
        <CardHeader className="text-center pb-4 pt-6">
          ...
          <CardTitle className="text-xl font-bold text-slate-800 mb-1">{plan.name}</CardTitle>
          {plan.description && (
            ...
            <div>
              ...
              <div className="space-y-1">
                {plan.planTimeSlots.map((slot) => (
                  <div key={slot.id} className="text-sm text-slate-600 bg-slate-50 px-2 py-1 rounded">
