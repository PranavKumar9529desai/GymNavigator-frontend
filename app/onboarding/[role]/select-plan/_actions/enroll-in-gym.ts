...
export async function enrollInGym(
	...
	try {
		...
		if (errorMessage.includes('already has an active membership')) {
			throw new Error(
				'You already have an active membership at this gym. Please contact support if you need to change plans.',
			);
		}

		if (errorMessage.includes('Invalid gym credentials')) {
			throw new Error(
				'Invalid gym credentials. Please scan the QR code again.',
			);
		}
