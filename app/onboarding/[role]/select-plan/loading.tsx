			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
					{Array.from({ length: 6 }).map((_, index) => (
						<div
							key={`plan-skeleton-${String.fromCharCode(65 + index)}`}
							className="relative rounded-lg border-2 border-slate-200 p-6 bg-white"
						>
							...
							<div className="mb-6">
								...
								<div className="space-y-2">
									{['feature-1', 'feature-2', 'feature-3'].map((featureId) => (
										<div
											key={`plan-${String.fromCharCode(65 + index)}-${featureId}`}
							...
							<div className="mb-6">
								...
								<div className="space-y-1">
									{['slot-1', 'slot-2'].map((slotId) => (
										<Skeleton
											key={`plan-${String.fromCharCode(65 + index)}-${slotId}`}
											className="h-4 w-24"
										/>
									))}
								</div>
							...
							<div className="space-y-1">
								{['info-1', 'info-2'].map((infoId) => (
									<Skeleton
										key={`plan-${String.fromCharCode(65 + index)}-${infoId}`}
										className="h-3 w-20"
									/>
								))}
							</div>
