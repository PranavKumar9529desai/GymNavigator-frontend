...
export default function AllergiesForm() {
  ...
  const handleSubmit = () => {
    ...
    else if (!allergies.some(allergy => allergy.selected) && !otherAllergy.trim().length) {
  ...
  const hasSelectedAllergies =
    allergies.some((allergy) => allergy.selected) || otherAllergy.trim().length > 0;
