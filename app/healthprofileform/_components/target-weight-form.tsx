...
export default function TargetWeightForm() {
	const { targetWeight, weight, setTargetWeight, nextStep, prevStep } =
		useHealthProfileStore();
	const [value, setValue] = useState<number>(
		targetWeight.value || weight.value || 65,
	);
	const [unit, setUnit] = useState<'kg' | 'lb'>(
		targetWeight.unit || weight.unit || 'kg',
	);
	...
	const handleDecrement = () => {
		setValue((prev) =>
			Math.max(prev - (unit === 'kg' ? 1 : 2), unit === 'kg' ? 40 : 88),
		);
	};
		...
		<div className="flex flex-col min-h-[60vh] justify-between">
			<div>
				...
				<p className="text-gray-500 mb-6">
					Set a realistic goal that you want to achieve
				</p>
