...
export function getActivityFactor(activityLevel: ActivityLevel): number {
  switch (activityLevel) {
    case 'sedentary': return 1.2;     // Sedentary (little exercise)
    case 'light': return 1.375;       // Lightly active (light exercise 1-3 days/week)
    case 'moderate': return 1.55;     // Moderately active (moderate exercise 3-5 days/week)
    case 'active': return 1.725;      // Very active (hard exercise 6-7 days/week)
    case 'veryActive': return 1.9;    // Extra active (very hard exercise & physical job or 2x training)
    default: return 1.2;              // Default to sedentary if unknown
  }
}
