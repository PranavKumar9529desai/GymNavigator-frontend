      ...
      <div className="w-full relative">
        ...
        <div className="w-full overflow-x-auto pb-3 scrollbar-hide">
          ...
          >
            {plan.schedules.map((schedule) => (
              ...
              >
                <div className={`flex items-center justify-center w-8 h-8 rounded-full mb-1.5 text-white ${
                  activeTab === schedule.dayOfWeek ? "bg-indigo-600 shadow-md" : "bg-indigo-500/80"
                }`}>
                  {schedule.dayOfWeek.slice(0, 1)}
                </div>
                <span className={`font-medium ${activeTab === schedule.dayOfWeek ? "text-indigo-700 dark:text-indigo-300" : ""}`}>
        ...
        <div className="flex justify-center gap-1 mt-1">
          {plan.schedules.map((schedule, index) => (
            <span 
              key={`dot-${schedule.dayOfWeek}`}
              className={`h-1.5 rounded-full transition-all ${
                activeTab === schedule.dayOfWeek ? "w-4 bg-indigo-600" : "w-1.5 bg-gray-300 dark:bg-gray-700"
              }`} 
              aria-hidden="true"
            />
          ))}
