...
function getWorkoutTitle(
	workoutPlan: WorkoutPlan | WorkoutPlanWithAlternativeNames,
): string {
	return (
		workoutPlan.name ||
...
function getWorkoutFocus(
	schedule: WorkoutSchedule | WorkoutScheduleWithAlternativeNames,
): string {
	return (
...
function getWorkoutDifficulty(workout: WorkoutHistoryItem): string {
	...
	const avgDuration =
		workout.workoutPlan.schedules.reduce(
							...
							>
								<div className="flex justify-between items-start mb-2">
									<div>
										<div className="flex items-center gap-2">
											...
											<div
												className={`text-xs px-2 py-0.5 rounded-full ${difficultyClass}`}
											>
												{difficulty}
											</div>
										</div>
								...
								<div className="flex flex-wrap gap-2 text-xs">
									...
									<div className="bg-muted px-2.5 py-1 rounded-full flex items-center gap-1">
										...
										<span>
											{Math.max(
												...item.workoutPlan.schedules.map(
													(s) => s.duration || 0,
												),
											)}{' '}
											min
										...
										<div className="mt-3 space-y-2">
											...
											<div className="space-y-1.5">
												{item.workoutPlan.schedules.map((day, idx) => {
													...
													const _icon = MUSCLE_ICONS[focus] || (
