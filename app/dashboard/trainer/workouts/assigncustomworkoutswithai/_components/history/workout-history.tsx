...
function getWorkoutTitle(workoutPlan: any): string {
  return workoutPlan.name || "Untitled Workout";
}

// Helper function to determine workout focus/target area
function getWorkoutFocus(schedule: any): string {
  return schedule.muscleTarget || "General";
}
...
function getWorkoutDifficulty(workout: WorkoutHistoryItem): string {
  ...
  const avgDuration = workout.workoutPlan.schedules.reduce(
    ...
    <div className="space-y-4">
      ...
      <div className={viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 gap-4" : "space-y-4"}>
        {history.map((item) => {
          ...
          const difficultyClass = DIFFICULTY_COLORS[difficulty as keyof typeof DIFFICULTY_COLORS] || DIFFICULTY_COLORS.default;
            ...
            >
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 text-xs mt-3">
                  ...
                  <div className="bg-muted px-2.5 py-1 rounded-full flex items-center gap-1">
                    ...
                    <span>
                      {Math.max(
                        ...item.workoutPlan.schedules.map((s) => s.duration || 0)
                      )}{" "}
                      min
                    </span>
                  </div>
              ...
              {expandedItemId === item.id && (
                <div className="p-4 pt-0 border-t border-dashed">
                    ...
                    <div className="mt-3 space-y-2">
                      ...
                      <div className="space-y-1.5">
                        {item.workoutPlan.schedules.map((day, idx) => {
                          ...
                          const icon = MUSCLE_ICONS[focus] || <Target className="h-3 w-3" />;
                  ...
                  {!isShowingDayExercises && item.conversationHistory &&
