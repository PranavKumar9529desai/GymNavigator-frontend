...
const generateId = () =>
	`workout-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
...
export const useWorkoutChatStore = create<WorkoutChatStore>((set, get) => ({
	...
	saveWorkoutToHistory: (workoutPlan, userId, userName) => {
		...
		if (typeof window !== 'undefined') {
			try {
				...
				const savedWorkoutsStr =
					localStorage.getItem('gym-navigator-saved-workouts') || '[]';
	...
	getSavedWorkouts: (userId) => {
		...
		try {
			const savedWorkoutsStr = localStorage.getItem(
				'gym-navigator-saved-workouts',
			);
			if (!savedWorkoutsStr) return [];

			const allSavedWorkouts = parseWithDates(
				savedWorkoutsStr,
			) as SavedWorkout[];
			console.log(
				'Found saved workouts in localStorage:',
				allSavedWorkouts.length,
			);
