...
export const useWorkoutChatStore = create<WorkoutChatStore>((set, get) => ({
  ...
  // Actions
  initializeConversation: (workoutPlan) => set({
    conversationHistory: [{
      type: 'ai',
      message: "Here's your personalized workout plan. Let me know if you'd like any modifications!",
      workout: workoutPlan,
      timestamp: new Date(),
    }],
    feedbackError: null,
  ...
  addAIMessage: (message, workoutPlan) => set((state) => ({
    conversationHistory: [
      ...state.conversationHistory,
      {
        type: 'ai',
  ...
  saveWorkoutToHistory: (workoutPlan, userId, userName) => {
    ...
    const normalizedWorkoutPlan: WorkoutPlan = {
      ...
      name: workoutPlan.name || (workoutPlan as any).title || "Untitled Workout",
      ...
      schedules: workoutPlan.schedules.map(schedule => ({
        ...
        muscleTarget: schedule.muscleTarget || (schedule as any).focus || "General",
