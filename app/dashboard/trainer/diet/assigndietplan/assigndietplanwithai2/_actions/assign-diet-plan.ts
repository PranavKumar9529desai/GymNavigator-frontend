...
export async function assignDietPlan({ userId, dietPlan, day }: AssignDietPlanParams) {
  try {
...
export async function assignWeeklyDietPlan({ 
  ...
  try {
    ...
    const formattedWeeklyPlans = Object.entries(weeklyPlans).map(([day, plan]) => ({
      day,
      plan: {
