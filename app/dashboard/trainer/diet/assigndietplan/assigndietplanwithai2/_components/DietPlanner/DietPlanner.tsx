...
import { assignDietPlan, assignWeeklyDietPlan } from '../../_actions/assign-diet-plan';
...
export function DietPlanner() {
    ...
    const [originalPlans, setOriginalPlans] = useState<Record<string, DietPlan> | null>(null);
        ...
        <div className="w-full space-y-6">
            ...
            {isPending && (
                <div className="flex flex-col justify-center items-center p-8 bg-white rounded-lg shadow-sm border border-blue-100">
                    ...
                    <p className="text-blue-700 font-medium">Generating weekly diet plan with native foods...</p>
                    <p className="text-sm text-blue-500 mt-2">This may take a moment as we create a culturally appropriate meal plan for the entire week</p>
                </div>
            )}
            ...
            {isAssigning && (
                <div className="flex flex-col justify-center items-center p-8 bg-white rounded-lg shadow-sm border border-blue-100">
                    ...
                    <p className="text-green-700 font-medium">Assigning diet plan to user...</p>
                    <p className="text-sm text-green-500 mt-2">Saving plan to the database and assigning to the user's profile</p>
                </div>
            )}
            ...
            {!isPending && !isAssigning && dietPlans && dietPlans[activeDay] && (
                <div className="mt-4 bg-white rounded-lg shadow-sm border border-blue-50 overflow-hidden">
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 border-b border-blue-100">
                        ...
                        <p className="text-sm text-blue-600">Native diet tailored to client's location and preferences</p>
                    </div>
                    <div className="p-4">
                        <DietDisplay 
            ...
            {!isPending && !isAssigning && !dietPlans && (
                <div className="bg-white rounded-lg shadow-sm border border-blue-50 p-6 text-center">
                    ...
                    <p className="text-sm text-gray-500 mb-4">Click "Generate Diet Plan" to create a customized plan with native foods based on the user's profile and location.</p>
                </div>
            )}
        </div>
    );
