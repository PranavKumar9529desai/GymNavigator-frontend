	...
	return useQuery({
		...
		queryFn: async () => {
			...
			const backendIdsSet = new Set(
				backendDiets.map((diet) => diet.id.toString()),
			);

			// Filter out local diets that already exist in backend
			const uniqueLocalDiets = localDiets.filter(
				(item) =>
					!backendIdsSet.has(
						typeof item.dietPlan.id === 'string'
							? item.dietPlan.id
...
export function useSaveDietPlanMutation() {
	...
	return useMutation({
		mutationFn: async ({
			...
			console.log(
				'dietPlanInput from the useSaveDietPlanMutation',
				dietPlanInput,
			);
