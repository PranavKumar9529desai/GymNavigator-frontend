...
export function DietPlanSelector({
	...
	const [selectedDietPlan, setSelectedDietPlan] = useState<DietPlan | null>(
		null,
	);
		...
		if (userProfile.HealthProfile) {
			...
			if (health.dietaryPreference) {
				if (health.dietaryPreference.toLowerCase().includes('vegetarian')) {
					...
					health.dietaryPreference.toLowerCase().includes('non-vegetarian')
				) {
	...
	const getDietType = (plan: DietPlan): string => {
		...
		if (
			planName.includes('veg') ||
			planDesc.includes('vegetarian') ||
			planDesc.includes('vegan')
		) {
			return 'vegetarian';
		}
		if (
			planName.includes('non-veg') ||
			planDesc.includes('non-vegetarian') ||
			planName.includes('chicken') ||
			planName.includes('meat') ||
			planName.includes('fish')
		) {
	...
	const filteredDietPlans = dietPlans.filter((plan) => {
		...
		if (dietTypeFilter !== 'all') {
			...
			matchesDietType =
				planDietType === dietTypeFilter || planDietType === 'mixed';
		}

		return (
			matchesSearch && matchesCreator && matchesMealTimes && matchesDietType
		);
	});
	...
	const getRecommendations = (plan: DietPlan) => {
		...
		const planDietType = getDietType(plan);
		const userDietType = health.dietaryPreference
			?.toLowerCase()
			.includes('vegetarian')
			? 'vegetarian'
			: 'non-vegetarian';
		...
		const dietTypeMatch =
			planDietType === userDietType || planDietType === 'mixed';
		...
		if (
			health.goal.toLowerCase().includes('weight loss') &&
		...
		if (
			health.goal.toLowerCase().includes('muscle') &&
			(plan.proteinPercent || 0) > 25
		) {
	...
	const handleConfirmSelection = async () => {
		...
		setIsAssigning(true);
		try {
			const result = await attachDietPlanToUser(
				userId,
				selectedDietPlan.id.toString(),
			);
			...
			if (result.success) {
				...
				toast({
					...
					description:
						result.message || 'Failed to assign diet plan. Please try again.',
					variant: 'destructive',
				});
			}
		} catch (error: unknown) {
				...
				<div className="flex items-center gap-3">
					...
					<div className="flex-1">
						...
						<CardDescription>
							...
							{dietPlans.length} plans available from your gym
						</CardDescription>
					</div>
							...
							{dietPlans.length === 0 ? (
								<div>
									...
									<p className="text-sm mt-1">
										Create some diet plans or check with other trainers in your
										gym.
									</p>
								</div>
							) : (
