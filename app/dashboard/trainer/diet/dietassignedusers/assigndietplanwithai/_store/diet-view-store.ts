...
interface DietViewStore {
	...
	setActiveDiet: (
		diet: { clientName: string; dietPlan: DietPlan } | null,
	) => void;
...
const generateId = () =>
	`diet-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
	...
	persist(
		(set, get) => ({
				...
				try {
					if (typeof window !== 'undefined') {
						localStorage.setItem(
							'gym-navigator-saved-diets',
							JSON.stringify(updatedDiets),
						);
					}
				} catch (error) {
			...
			getSavedDiets: (userId) => {
				...
				try {
					const savedDietsStr = localStorage.getItem(
						'gym-navigator-saved-diets',
					);
