...
import type {
	AssignWeeklyDietPayload,
	AssignSingleDietPayload,
} from '../../diet-types'; // Adjust import path as needed
...
export async function assignDietPlan({
	...
	day: string;
	dietPlan: AssignSingleDietPayload['dietPlan'];
}) {
	try {
...
export async function assignWeeklyDietPlan({
	...
	weeklyPlans: Record<string, AssignSingleDietPayload['dietPlan']>;
}) {
	try {
		const trainerAxios = await TrainerReqConfig();
		...
		const payload = {
			userId: Number(userId),
			weeklyPlans: Object.entries(weeklyPlans).map(([day, plan]) => ({
				day,
				plan,
			})),
		};
		const response = await trainerAxios.post(
			'/diet/createandassignweekly',
			payload,
		);
		if (response.data.success) {
			return {
		...
		throw new Error(
			response.data.error || 'Failed to assign weekly diet plans',
		);
	} catch (error) {
