...
const SaveGroceryListParamsSchema = z.object({
  ...
  groceryList: z.custom<GroceryListResponse>((val) => 
    val && typeof val === 'object' && 'categories' in val, 
...
export async function saveGroceryList(
  ...
  try {
    const { timeFrame, groceryList } = SaveGroceryListParamsSchema.parse(params);
