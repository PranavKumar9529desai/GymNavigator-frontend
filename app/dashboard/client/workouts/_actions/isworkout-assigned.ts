...
export const checkIsWorkoutAssigned = async (): Promise<IsWorkoutAssignedData> => {
	try {
		...
		if (!response.data.success || typeof response.data.data?.isAssigned === 'undefined') {
			console.error('Failed to fetch workout assignment status:', response.data.error);
