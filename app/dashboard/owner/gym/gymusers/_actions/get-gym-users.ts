...
export async function getGymUsers(): Promise<GymUsersApiResponse> {
  ...
  try {
    ...
    return {
      ...
      counts: {
        totalUsers: typeof counts?.totalUsers === 'number' ? counts.totalUsers : 0,
        totalTrainers: typeof counts?.totalTrainers === 'number' ? counts.totalTrainers : 0,
        totalClients: typeof counts?.totalClients === 'number' ? counts.totalClients : 0,
      },
