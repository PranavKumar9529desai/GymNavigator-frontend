			...
			<CardContent className="flex items-center gap-3 p-0 pt-6">
				<Avatar className="h-10 w-10">
					{type === 'trainer' && (user as GymTrainer).image ? (
						<AvatarImage
							src={(user as GymTrainer).image ?? ''}
							alt={user.name}
						/>
					) : (
						<AvatarFallback className="bg-indigo-100/50 text-indigo-500">
							{user.name
								.split(' ')
								.slice(0, 2)
								.map((n) => n[0])
				...
				{type === 'user' ? (
					<Badge
						variant="outline"
						className="text-xs px-2 py-0.5 border-blue-100 text-blue-600"
					>
					...
					<Badge
						variant="outline"
						className="text-xs px-2 py-0.5 border-indigo-100 text-indigo-600 flex items-center gap-1"
					>
