...
export function GymUserMobileCard({ user, type }: GymUserMobileCardProps) {
  ...
  const handleViewProfile = () => {
    router.push(`/dashboard/owner/gymusers/${user.id}`);
  };
  return (
    <Card className="bg-gradient-to-br from-blue-50/30 via-white to-indigo-50/20 p-3 flex flex-col gap-2">
      <CardContent className="flex items-center gap-3 p-0">
        ...
        {type === 'user' ? (
          ...
          <Badge variant="outline" className="text-xs px-2 py-0.5 border-indigo-100 text-indigo-600 flex items-center gap-1">
            <Dumbbell className="h-3 w-3" />{(user as GymTrainer).assignedClients} Clients
          </Badge>
        )}
      </CardContent>
