...
export default function GymUsersView({
	data,
	role,
	setRole,
}: GymUsersViewProps) {
	...
	const userColumns = [
		...
		},
		{
			...
			cell: ({ row }: { row: Row<GymUser> }) => {
				...
				return <MoreActions actions={actions} label="User Actions" />;
			},
			enableSorting: false,
			enableHiding: false,
		},
				...
				const actions = [
					{
						...
						icon: UserIcon,
						onClick: () =>
							router.push(`/dashboard/owner/gym/gymusers/${trainer.id}`),
						className: 'text-black',
						tooltip: `View ${trainer.name}'s profile`,
					},
					// Add more actions here if needed
				];
				return <MoreActions actions={actions} label="Trainer Actions" />;
			},
			enableSorting: false,
			enableHiding: false,
		},
	...
	return (
		<div className="p-4">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
				...
				<StatusCard
					title="Trainers"
					value={data.counts.totalTrainers}
					icon={Dumbbell}
					gradient="green"
				/>
				<StatusCard
					title="Clients"
					value={data.counts.totalClients}
					icon={User}
					gradient="yellow"
				/>
			</div>
			...
			<div className="mb-4 flex flex-col md:flex-row gap-3 md:gap-4 items-stretch">
				...
				<div className="w-full md:w-auto">
					<Select
						value={role}
						onValueChange={(v) => setRole(v as 'trainer' | 'client')}
					>
