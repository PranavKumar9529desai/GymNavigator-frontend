...
export default function RevenueChart({ breakdowns }: RevenueChartProps) {
	...
	return (
		<Card className="border-blue-200 bg-white/80 backdrop-blur-sm shadow-sm hover:shadow-md transition-shadow">
			<CardHeader>
				<CardTitle className="text-lg font-semibold text-slate-800">Revenue by Plan</CardTitle>
			</CardHeader>
			<CardContent>
				<div className="h-64">
				...
				<div className="mt-4 space-y-2">
					{breakdowns.revenueByPlan.map((plan, index) => {
						const totalRevenue = breakdowns.revenueByPlan.reduce((sum, p) => sum + p.value, 0);
						const percentage = totalRevenue > 0 ? (plan.value / totalRevenue) * 100 : 0;
						return (
							<div key={plan.label} className="flex items-center justify-between p-2 bg-gradient-to-r from-slate-50/50 to-blue-50/50 rounded-lg border border-slate-100">
								<div className="flex items-center space-x-3">
									...
									<div>
										...
										<div className="text-xs text-slate-500">{plan.count || 0} members</div>
									</div>
								</div>
								<div className="text-right">
									<div className="font-semibold text-slate-800">{formatCurrency(plan.value)}</div>
