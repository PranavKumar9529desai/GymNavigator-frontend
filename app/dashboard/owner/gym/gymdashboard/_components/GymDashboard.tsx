						...
						<ul className="space-y-1">
							{recentActivities.recentAttendance.map((att) => (
								<li key={att.id} className="text-xs text-slate-600">
									...
									checked in at {new Date(att.scanTime).toLocaleTimeString()}
								</li>
							))}
						</ul>
					</div>
			...
			<CardSection title="Gym Details">
				<div className="flex flex-wrap gap-4 items-center text-xs text-slate-700">
					<div className="flex flex-col">
						...
						<span>
							{gymDetails.address?.street}, {gymDetails.address?.city},{' '}
							{gymDetails.address?.state}, {gymDetails.address?.postalCode}
						</span>
					</div>
					<div className="flex flex-col">
					...
					<div className="flex flex-col">
						...
						<span>
							{gymDetails.pricingPlans.map((plan) => plan.name).join(', ')}
						</span>
					</div>
				</div>
			</CardSection>
...
// @biome-ignore lint/correctness/noUnusedVariables: StatCard is kept for future use
