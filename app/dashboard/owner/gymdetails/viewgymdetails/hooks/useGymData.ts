	...
	return useMutation({
		...
		onMutate: async (newAmenities: UpdateAmenitiesRequest) => {
			...
			queryClient.setQueryData(
				gymQueryKeys.tabData(),
				(old: GymTabData | undefined) => {
					if (!old) return old;
					return {
						...old,
						amenities: {
							...old.amenities,
							selectedAmenities: convertAmenityKeysToFormData(
				...
				(old: GymTabData | undefined) => {
					...
					return {
						...
						pricing: {
							...
							additionalServices:
								newPricing.additionalServices ||
								old.pricing?.additionalServices ||
								[],
						},
					};
				},
...
function convertAmenityKeysToFormData(
	amenityKeys: string[],
