...
import {
	TrainerUserBarChart,
	type Trainer,
} from '../charts/trainer-user-bar-chart';

function UserCard({
	type,
	number,
}: { type: 'trainers' | 'users'; number: number }) {
	return (
		<Card className="w-full max-w-xs shadow-md h-24 flex items-center justify-center bg-gradient-to-br from-blue-50 to-white border-blue-100 dark:from-blue-950 dark:to-gray-900 dark:border-blue-900">
			<div className="flex items-center justify-center h-20 p-3 gap-3 w-full">
				...
				<div className="flex flex-col justify-center ml-2">
					<p className="capitalize text-gray-700 dark:text-gray-200 text-xs sm:text-sm font-medium tracking-wide">
						{type}
					</p>
					<p className="font-bold text-gray-950 dark:text-white text-xl sm:text-2xl">
						{number}
					</p>
				</div>
			</div>
...
const defaultTrainersData: Trainer[] = [
	...
	},
	{
		id: 2,
		name: 'Trainer 2',
		assignedClients: [{ id: 3, name: 'Client C' }],
	},
	{
		id: 3,
		name: 'Trainer 3',
	...
	},
	{
		id: 3,
		name: 'Trainer 3',
		assignedClients: [{ id: 6, name: 'Client F' }],
	},
	{
		id: 3,
		name: 'Trainer 3',
...
export function OverviewTab({
	...
	const totalUsers = trainersData.reduce(
		(total, trainer) => total + trainer.assignedClients.length,
		0,
	);
