...
export function AmenitiesTab({
  ...
  const getCategoryColor = (categoryKey: string) => {
    ...
    return colorMap[categoryKey as keyof typeof colorMap] || 'from-gray-500 to-gray-600'
  }
  ...
  const getAvailabilityStats = (category: AmenityCategoryDefinition) => {
    if (allSelected) return { available: category.amenities.length, total: category.amenities.length }
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {filteredCategories.map((category, index) => {
            ...
            const availabilityPercentage = (stats.available / stats.total) * 100
            ...
            return (
              ...
              >
                <Card className="h-full border-0 shadow-lg bg-gradient-to-br from-white to-gray-50/50 hover:shadow-xl transition-all duration-300 group">
                  <CardHeader className={`bg-gradient-to-r ${getCategoryColor(category.key)} text-white relative overflow-hidden`}>
                    ...
                    <div className="mt-3">
                      <div className="flex items-center justify-between text-sm text-white/80 mb-1">
                        ...
                        <span>{stats.available} of {stats.total}</span>
                      </div>
                      <Progress 
                  ...
                  <CardContent className="p-6">
                    <div className={viewMode === 'grid' ? "space-y-3" : "grid grid-cols-2 gap-2"}>
                      {category.amenities.map((amenity, amenityIndex) => {
                        ...
                        return (
                          <motion.div
                            ...
                            transition={{ duration: 0.2, delay: amenityIndex * 0.05 }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {allSelected 
                  ? categories.reduce((acc, cat) => acc + cat.amenities.length, 0)
                  : Object.values(selectedAmenities || {}).reduce((acc, arr) => acc + arr.length, 0)
                }
              </div>
