...
import type { GymData, FitnessPlan, AdditionalService, PricingFormData } from "../../types/gym-types";
...
function SortablePlanCard({ plan, index, onUpdate, onRemove }: {
  plan: FitnessPlan;
  index: number;
  ...
  const SelectedIcon = planIcons.find(icon => icon.name === localPlan.icon)?.icon || Dumbbell;
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div 
                  ...
                  style={{ backgroundColor: `${localPlan.color}20` || '#f3f4f620' }}
                >
                  <SelectedIcon 
                    className="h-5 w-5" 
                  ...
                  {planColors.map((color) => (
                    <button
                      ...
                      className={`w-8 h-8 rounded-full border-2 ${
                        localPlan.color === color ? 'border-gray-900' : 'border-gray-200'
                      }`}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => updateLocalPlan({ isFeatured: checked, popular: checked })}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => updateLocalPlan({ maxMembers: e.target.value ? parseInt(e.target.value) : undefined })}
...
export function PricingEditForm({ data, onDataChange, onSave }: PricingEditFormProps) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = plansFormData.findIndex((plan) => 
        (plan.id || `plan-${plansFormData.indexOf(plan)}`) === active.id
      );
      const newIndex = plansFormData.findIndex((plan) => 
  ...
  const updateAdditionalService = (index: number, field: keyof AdditionalService, value: string) => {
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="plans" className="space-y-4">
          ...
          {plansFormData.length === 0 ? (
            ...
            >
              <SortableContext
                items={plansFormData.map((plan, index) => plan.id || `plan-${index}`)}
          ...
          {isLoadingServices ? (
            ...
            <Card className="p-8 text-center">
              <div className="space-y-3">
                ...
                <p className="text-gray-500">Add services like personal training or day passes</p>
