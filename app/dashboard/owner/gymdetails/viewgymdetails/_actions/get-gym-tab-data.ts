...
export async function getOverviewData(): Promise<{
  ...
  try {
    ...
    const response: AxiosResponse<TrainersApiResponse> = await ownerAxios.get('/trainer/fetchtrainers');
    ...
    if (response.data.msg === 'success' && response.data.trainers) {
      ...
      const trainersData: Trainer[] = response.data.trainers.map(trainer => ({
        id: trainer.id,
        name: trainer.name,
        assignedClients: trainer.users?.map(user => ({
          id: user.id,
          name: user.name,
        })) || [],
      }));
...
export async function getPricingData(): Promise<{
  ...
  try {
    ...
    const response: AxiosResponse<PricingApiResponse> = await ownerAxios.get('/gym/pricing');
...
export async function getAllGymTabData(): Promise<{
  ...
  try {
    ...
    const [overviewResult, amenitiesResult, locationResult, pricingResult] = await Promise.all([
      getOverviewData(),
      getAmenitiesData(),
      getLocationData(),
      getPricingData(),
