...
export function useGetCoordinatesFromLocation() {
  ...
  const getCoordinates = useCallback(async (addressData: AddressInput): Promise<Coordinates | null> => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      console.log("Geocoding response from get-co-ordinates-from-location.ts", data);
      ...
      setError(err.message || 'Unknown error occurred while geocoding');
      return null;
    } finally {
      setLoading(false);
    }
