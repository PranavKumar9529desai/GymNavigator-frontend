...
export async function getUserProfile(
	...
	try {
		...
		const response = await clientAxios.get(`/profile/userprofile/${userId}`);
		...
		const userProfileData: UserProfileResponse = {
			...
			overview: data.overview
				? {
						attendanceData:
							data.overview.attendanceData || generateMockAttendanceData(),
						workoutTypes:
							data.overview.workoutTypes || generateMockWorkoutTypes(),
						stats: {
			...
			health: data.health
				? {
						...
						weightProgress:
							data.health.weightProgress || generateMockWeightProgress(),
					}
				: null,
			...
			attendance: data.attendance
				? {
						weeklyAttendance:
							data.attendance.weeklyAttendance ||
							generateMockWeeklyAttendance(),
