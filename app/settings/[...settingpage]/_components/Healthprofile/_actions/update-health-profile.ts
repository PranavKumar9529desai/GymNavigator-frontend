  ...
  try {
    ...
    const flattenedData = {
      ...
      nonVegDays: profileData.Dietary?.nonVegDays ? 
        JSON.stringify(profileData.Dietary.nonVegDays) : undefined,
      ...
      dietaryRestrictions: profileData.Dietary?.dietaryRestrictions ? 
        JSON.stringify(profileData.Dietary.dietaryRestrictions) : undefined,
      
      // Medical
      allergies: profileData.Medical?.allergies ? 
        JSON.stringify(profileData.Medical.allergies) : undefined,
      otherAllergy: profileData.Medical?.otherAllergy,
      medicalConditions: profileData.Medical?.medicalConditions ? 
        JSON.stringify(profileData.Medical.medicalConditions) : undefined,
    ...
    const response = await clientAxios.put('/profile/updatehealthprofile', flattenedData);
