 ...
 */
export default function MealTimingsDisplay({ timings, mealTimes }: MealTimingsDisplayProps) {
  ...
  const getMealColor = (mealName: string, index: number) => {
    ...
    const colors = ["bg-blue-100 text-blue-800", "bg-purple-100 text-purple-800", "bg-pink-100 text-pink-800"];
  ...
  if (mealTimingsList.length === 0) {
    return (
      <div className="text-sm text-gray-500 italic p-4 bg-gray-50 rounded-md flex items-center justify-center">
        ...
        {mealTimes ? `${mealTimes} meals per day` : 'No meal schedule information available'}
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-medium text-gray-700">Daily Meal Schedule</h4>
        </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`h-10 w-10 rounded-full ${colorClass.split(' ')[0]} flex items-center justify-center`}>
      ...
      <div className="mt-3 pt-2 border-t border-dashed border-gray-200">
        <p className="text-xs text-gray-500 italic">
          These meal timings help trainers optimize your nutrition and workout schedule.
        </p>
      </div>
    </div>
  );
