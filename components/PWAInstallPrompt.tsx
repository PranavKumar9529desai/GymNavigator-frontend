...
const PWAInstallPrompt: React.FC = () => {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      const firstVisit = parseInt(localStorage.getItem(FIRST_VISIT_KEY) || '0', 10);
      ...
      if (visits >= MIN_VISITS) {
        setShowPrompt(true);
      } else if (firstVisit && (now - firstVisit >= PROMPT_DELAY_SECONDS * 1000)) {
        setShowPrompt(true);
      } else {
        // Set timer for time-based prompt
        if (timerRef.current) clearTimeout(timerRef.current);
        const delay = firstVisit ? Math.max(0, PROMPT_DELAY_SECONDS * 1000 - (now - firstVisit)) : PROMPT_DELAY_SECONDS * 1000;
        timerRef.current = setTimeout(() => {
          // Re-check conditions before showing
          const visitsNow = parseInt(localStorage.getItem(VISIT_KEY) || '0', 10);
          const dismissedAtNow = parseInt(localStorage.getItem(DISMISS_KEY) || '0', 10);
          const now2 = Date.now();
          const dismissedRecentlyNow = dismissedAtNow && (now2 - dismissedAtNow < DISMISS_DAYS * 24 * 60 * 60 * 1000);
